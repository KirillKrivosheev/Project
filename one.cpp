...
public:
	Figure fig;
	float moveTimer;//добавили переменную таймер дл€ будущих целей
	int health;
	bool life, isMove, onGround;
	Texture texture;
	Sprite sprite;
	String name;//враги могут быть разные, мы не будем делать другой класс дл€ различающегос€ врага.всего лишь различим врагов по имени и дадим каждому свое действие в update в зависимости от имени
...
public:
	...
	void control() {
		if (Keyboard::isKeyPressed) {//если нажата клавиша
			if (Keyboard::isKeyPressed(Keyboard::Left)) {//а именно лева€
	...
	{
		for (int i = fig.y / 32; i < (fig.y + fig.h) / 32; i++)//проходимс€ по элементам карты
			for (int j = fig.x / 32; j < (fig.x + fig.w) / 32; j++)
			{
				if (TileMap[i][j] == '0')//если элемент наш тайлик земли? то
				{
					if (Dy > 0) { fig.y = i * 32 - fig.h;  fig.dy = 0; }//по Y вниз=>идем в пол(стоим на месте) или падаем. ¬ этот момент надо вытолкнуть персонажа и поставить его на землю, при этом говорим что мы на земле тем самым снова можем прыгать
					if (Dy < 0) { fig.y = i * 32 + 32;  fig.dy = 0; }//столкновение с верхними кра€ми карты(может и не пригодитьс€)
...
public:
	Enemy(Image &image, float X, float Y, int W, int H, String Name) :Entity(image, X, Y, W, H, Name) {
		if (name == "EasyEnemy") {
			sprite.setTextureRect(IntRect(0, 0, fig.w, fig.h));
			fig.dx = 0.2;//даем скорость.этот объект всегда двигаетс€
			fig.velocity.x = 0.2;
			fig.fi = acos(0);
			fig.velocity.y = 0;
			fig.speed = 0.2;
	...
	void checkCollisionWithMap(float Dx, float Dy)//ф ци€ проверки столкновений с картой
	{
		for (int i = fig.y / 32; i < (fig.y + fig.h) / 32; i++)//проходимс€ по элементам карты
			for (int j = fig.x / 32; j<(fig.x + fig.w) / 32; j++)
			{
				if (TileMap[i][j] == '0')//если элемент наш тайлик земли, то
				{
					if (Dy>0) { fig.y = i * 32 - fig.h; }//по Y вниз=>идем в пол(стоим на месте) или падаем. ¬ этот момент надо вытолкнуть персонажа и поставить его на землю, при этом говорим что мы на земле тем самым снова можем прыгать
					if (Dy<0) { fig.y = i * 32 + 32; }//столкновение с верхними кра€ми карты(может и не пригодитьс€)
